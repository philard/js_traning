...
function change() {
    ...
    if (state = urlParams(location.search.substr(1)), state.source !== e && state.source) source.property("value", state.source), getURL(state.source, function(e) {
        processText(e)
    }), hideHelp();
...
function scroll() {
    var e = page.datum();
    page.attr("y", e.y = Math.max(0, Math.min(height - e.h, height * this.scrollTop / (textViewer.rowHeight() * lines.length))))
}

function drag(e) {
    e.y = Math.max(0, Math.min(height - e.h - 1, d3.event.y)), text.node().scrollTop = e.y * textViewer.rowHeight() * lines.length / height, page.attr("y", e.y)
}
...
function(e) {
    function t() {
        function e(a) {
            ...
            if (s) {
                ...
                g.enter().append("tspan").text(function(e, t) {
                    ...
                    var n, r = d3.event;
                    r.ctrlKey || r.altKey || r.shiftKey ? (a.call(e.prefix([t.token.token])), n = t.token.token, t.data = b) : (n = f(t.data), o(t.data), e(a)), E.prefix({
                        keyword: n,
                        tree: t.data
                    })
                ...
                v.attr("visibility", null).transition().duration(U).attr("d", I), v.enter().insert("path", "a").attr("class", "link").attr("d", I).attr("visibility", "hidden").transition().delay(U).attr("visibility", null), v.exit().remove(), U = 500, n(b)
            }
        }

        function t(e) {
            var t = e.count / b.count * k;
            return e.children && e.children.length ? "0px" | (t > 30 ? 30 + S(e.count - 30 * b.count / k) : t - 1) : Math.max(0, Math.min(15, t - 1))
        }
        ...
        var y, k, w, b, T = 5,
            ...
            U = 0;
        return e.tokens = function(t) {
            if (!arguments.length) return _;
            _ = t, w = {};
            for (var n, r = 0, i = _.length; i > r; ++r) n = M.call(e, _[r], r), (w.hasOwnProperty(n) ? w[n] : w[n] = []).push(r);
            return e
        }, e.prefix = function(t) {
            return arguments.length ? (R = t.slice(), b = a(_, L ? R[R.length - 1] : R[0], L ? R.reverse() : R), e) : R
        }, e.root = function() {
            return b
...
var width, height, tree = wordtree().on("prefix", function(e) {
        text.call(textViewer);
        var t = state.prefix = e.keyword;
        keyword.property("value", t), url({
            prefix: t
        }), refreshText(e.tree)
    }),
...
}), d3.select("#sort").selectAll("option").data(["frequency", "occurrence"]).enter().append("option").attr("value", String).text(String), d3.select("#reverse").property("checked", +state.reverse).on("change", function() {
    url({
        reverse: this.checked ? 1 : 0
    }), change()
}), d3.select("#phrase-line").property("checked", +state["phrase-line"]).on("change", function() {
    url({
        "phrase-line": this.checked ? 1 : 0
    }), change()
