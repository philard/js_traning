...
exports.processText = function processText(e) {
    ...
    for (var linesIndex = []; t =exports.re.exec(e);) {
        var nonTextSeparator = e.substring(n, t.index);
        if(/\r\n\r\n|\r\r|\n\n/.test(nonTextSeparator)) {
            lines.push(linesIndex, [])
          linesIndex = [];
            o = t[0].length;
        } else {
            o += t[0].length + !!nonTextSeparator.length;
            if(o > msInRightBar()) {
                o = t[0].length;
                lines.push(linesIndex);
                linesIndex = [];
